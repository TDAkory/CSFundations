...
1. **入队触发条件**
   - 用户新建 Pod 且未指定 `nodeName`；
   - Node 故障导致已调度 Pod 被驱逐，重新进入调度队列；
   - 亲和性/污点策略变更，触发 Pod 重新调度。
2. **队列分层与排序**
   - **活跃队列**：存放高优先级待调度 Pod，调度器优先处理此队列，保证高优先级任务（如核心服务）优先调度；
   - **备用队列**：存放低优先级 Pod，活跃队列处理完毕后才会消费此队列；
   - **排序规则**：通过 `QueueSort` 插件实现，默认按 **Pod 优先级**（`spec.priorityClassName`）降序排列，优先级越高的 Pod 越先被调度。
